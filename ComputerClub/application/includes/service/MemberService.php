...
require_once 'DB.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/ComputerClub/application/includes/entity/Member.php';
...
class MemberService extends DB {
    ...
    private function getMember($memberID) {
        $memberExist = $this->em->getRepository('Member')->findOneBy(array('memberID' => $memberID));
        return $memberExist === null ? 0 : 1;
    }

    //Method to retrieve a specific member
    public function getMemberByID($memberID) {
        $member = $this->em->getRepository('Member')->findOneBy(array('memberID' => $memberID));
