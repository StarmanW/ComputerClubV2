...
require_once 'DB.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/ComputerClub/application/includes/entity/Item.php';
...
class ItemService extends DB {
    ...
    private function getItem($itemID) {
        $itemExist = $this->em->getRepository('Item')->findOneBy(array('itemID' => $itemID));
        return $itemExist === null ? 0 : 1;
    }

    //Method to retrieve a specific item
    public function getItemByID($itemID) {
        $itemID = $this->em->getRepository('Item')->findOneBy(array('itemID' => $itemID));
        return $itemID === null ? 0 : $itemID;
    }

    //Method to retrieve all Items
    public function getAllItems() {
        $items = $this->em->getRepository('Item')->findAll();
